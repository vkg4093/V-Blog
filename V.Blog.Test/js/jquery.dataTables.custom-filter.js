(function ($) {
    jQuery.fn.dataTableWithFilter = function (settings) {
        ...
        $("#" + filterOptions.advanceSearchButton).click(function () {
            searchCriteria = [];
            var searchContainer = $("#" + filterOptions.advanceSearchContainer);
            searchContainer.find('input[type="text"][value!=""],input[type="radio"]:checked,input[type="checkbox"]:checked,textarea[value!=""],select[value!=""]').each(function () {
                // all textboxes, radio buttons, checkboxes, textareas, and selects that actually have a value associated with them
                var element = $(this), value = element.val();
                if (typeof value === "string") {
        ...
        $("#" + filterOptions.filterButton).click(function () {
            searchCriteria = [];
            var searchContainer = $("#" + filterOptions.searchContainer);
            searchContainer.find('input[type="text"][value!=""],input[type="hidden"][value!=""],input[type="radio"]:checked,input[type="checkbox"]:checked,textarea[value!=""],select[value!=""]').each(function () {
                // all textboxes, radio buttons, checkboxes, textareas, and selects that actually have a value associated with them
                var element = $(this), value = element.val();
                if (typeof value === "string") {
                    searchCriteria.push({ "name": element.attr("name"), "value": value });
            ...
            if (filterOptions.filterButton == "SaveSectionBtn") {
                var isValid = ValidateDiscription();
                if (isValid == true)
                    $dataTable.fnDraw();
            }
            else {
